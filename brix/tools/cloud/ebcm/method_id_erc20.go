...
var (
	methodDeploy0 = cMethodIDData{
		...
		parseDeploy: func(wclient IClient, rs abi.RESULT, item *TransactionBlock) {
			item.ContractAddress = wclient.ContractAddressNonce(item.From, item.Nonce)
		},
	}
...
func (my cMethodIDData) _parseErc20Input(wclient IClient, item *TransactionBlock) bool {
	...
	if len(input) > len(my.MethodID) {
		...
		if my.parseInputABM != nil {
			...
			my.parseDeploy(wclient, cdata, item)
		}
	}
...
func CheckMethodERC20(wclient IClient, input string, item *TransactionBlock) {
	...
	for _, erc := range methodERC20s {
		isChecked = erc._parseErc20Input(wclient, item)
		if isChecked {
			break
		}
	}
