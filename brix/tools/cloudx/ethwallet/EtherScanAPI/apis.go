...
func GetEtherTransactions(cfg Config, depositAddress string, startBlock string, endBlock ...string) *ethwallet.ResponeTransactional {
	...
	txs := result.(*ethwallet.ResponeTransactional)
...
func GetTokenTransactions(cfg Config, depositAddress string, startBlock string, endBlock ...string) *ethwallet.ResponeTransactional {
	fixedEndblock := endblockNumber
	if len(endBlock) > 0 {
		fixedEndblock = endBlock[0]
	}
...
func GetTransactionAll(cfg Config, depositAddress string, startBlock string, endBlock ...string) *ethwallet.ResponeTransactional {
	fixedEndblock := endblockNumber
	if len(endBlock) > 0 {
		fixedEndblock = endBlock[0]
	}

	var txs *ethwallet.ResponeTransactional
	resultETH := getService(cfg).getEtherTransactions(depositAddress, startBlock, fixedEndblock).Execute().Body()
	if resultETH != nil {
		txs = resultETH.(*ethwallet.ResponeTransactional)
		if txs.IsSuccess() {
			for i := 0; i < len(txs.Result); i++ {
				txs.Result[i].ContractAddress = "eth"
				txs.Result[i].IsContract = false
...
func GetContractTransactions(cfg Config, contract string, page, offset string) *ethwallet.ContractTransactional {
	result := getService(cfg).getContractTransactions(contract, page, offset).Execute().Body()
	if result == nil {
		return nil
	}
	txs := result.(*ethwallet.ContractTransactional)
	txs.TxSort()
	return txs
}

// GetContractTxByBlock : 컨트랙 주소로 트랙젝션 조회 (블럭 단위)
func GetContractTxByBlock(cfg Config, contract string, startBlock string, endblock ...string) *ethwallet.ContractTransactional {
	startBlock = strings.TrimSpace(startBlock)

	eb := "9999999999999"
	if len(endblock) > 0 {
		var typeErr error
		if jmath.NewBigDecimal(endblock[0], &typeErr); typeErr == nil {
			if jmath.CompareTo(endblock[0], startBlock) > 0 {
				eb = strings.TrimSpace(endblock[0])
			}
		}
	}
...
func TokenBalance(cfg Config, contract, address string) *jmath.BigDecimal {
...
func TokenBalanceString(cfg Config, contract, address string) (string, error) {
	...
	return r.ResultValue().ToString(), nil
}

// ETHBalance : ETH 잔액 조회
func ETHBalance(cfg Config, address string) *jmath.BigDecimal {
