...
import (
	...
	"txscheduler/brix/tools/database/mongo/tools/sm"
...
func ConsoleQueryCmd3(cdb *CDB, node zzp.Normalizer) (resultMessage string) {
	...
	case "removeAll":
		col_action(func(c Collection) {
			if fQuery, err := text2Json(params[0]); err == nil {
				info, err := c.RemoveAll(fQuery)
				queryLog(jsonString(info), err)
			}
		})
	...
	case "updateAll":
		col_action(func(c Collection) {
			...
			if info, err := c.UpdateAll(fQuery, dQuery); err != nil {
				queryLog("updateAll_error :", err)
			} else {
				queryLog("updateAll_result :", jsonString(info))
				queryLog("< END >")
			}
		})
	...
	case "update":
		col_action(func(c Collection) {
			...
			if err := c.Update(fQuery, dQuery); err != nil {
				queryLog("update_error :", err)
			} else {
	...
	case "upsert":
		col_action(func(c Collection) {
			...
			if info, err := c.Upsert(fQuery, dQuery); err != nil {
				queryLog("upsert_error :", err)
			} else {
				queryLog("upsert_result :", jsonString(info))
				queryLog("< END >")
			}
		})
