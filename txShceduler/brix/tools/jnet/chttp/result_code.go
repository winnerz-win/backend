...
//ResultJSON :
func ResultJSON(w http.ResponseWriter, status int, v interface{}) {
	Renderer.JSON(w, status, v)
}

//ResultText :
func ResultText(w http.ResponseWriter, status int, text string) {
	Renderer.Text(w, status, text)
}

//ResultData :
func ResultData(w http.ResponseWriter, status int, data []byte) {
	Renderer.Data(w, status, data)
}
