...
func MakeGasPrice(gas_wei string) GasPrice {
	return GasPrice{
		Tip: jmath.BigInt(gas_wei),
		Gas: jmath.BigInt(gas_wei),
	}
}
func MakeGasPriceGWEI(gwei string) GasPrice {
	val := GWEI(gwei).ToWEI().String()
	return GasPrice{
		Tip: jmath.BigInt(val),
		Gas: jmath.BigInt(val),
	}
}
func (my GasPrice) GET_GAS_GWEI() string { return WEIToGWEI(my.GET_GAS_WEI()).String() }

////

func (my GasPrice) GET_GAS_WEI() string { return jmath.VALUE(my.Gas) }
func (my GasPrice) GET_GAS_ETH() string { return WeiToETH(my.GET_GAS_WEI()) }

func (my *GasPrice) MUL_VALUE(v any) {
	my.Tip = jmath.BigInt(jmath.DOTCUT(jmath.MUL(my.Tip, v), 0))
	my.Gas = jmath.BigInt(jmath.DOTCUT(jmath.MUL(my.Gas, v), 0))
}

func (my *GasPrice) AddGWEI_ALL(gwei any) {
	wei := GWEI(jmath.VALUE(gwei)).ToWEI().String()
	my.Tip = jmath.BigInt(jmath.ADD(wei, my.Tip))
	my.Gas = jmath.BigInt(jmath.ADD(wei, my.Gas))
}
func (my *GasPrice) AddGWEI_TIP(gwei any) {
	wei := GWEI(jmath.VALUE(gwei)).ToWEI().String()
