...
func (my awsInfo) GetURLList() []string    { return []string{my.clusterEndpoint} }
...
const (
	...
	connectionStringTemplate = "mongodb://%s:%s@%s/sample-database?tls=%v&replicaSet=rs0&readpreference=%s&retryWrites=%v"
)

func NewAWSTLS(clusterEndpoint, username, password, caFilePath, readPreference string, retryWrites, isTls bool) *CDB {
...
func newSessionAWS(info awsInfo) *dbSession {
	...
	if info.isTls {
		...
		if err != nil {
			log.Fatalf("Failed to create client to tls: %v", err)
		}
