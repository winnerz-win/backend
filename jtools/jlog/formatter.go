...
func (f *Formatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	is_text_color_mode := false
	text, color := prefix_message_color(message)
	if f.IsConsole && color != 0 {
		is_text_color_mode = true
		b.WriteString("\033[0;" + fmt.Sprint(color) + "m")
	}
	b.WriteString(text)
